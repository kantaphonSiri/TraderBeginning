...
def analyze_coin_ai(symbol, df, live_rate, invest_amount):
    try:
        ...
        # แก้ปัญหา Column MultiIndex ของ yfinance
...
if sheet:
    ...
    if recs:
        # แก้ปัญหา ArrowInvalid โดยการล้างข้อมูลว่างก่อนสร้าง DataFrame
        df_perf = pd.DataFrame(recs)
        # บังคับให้คอลัมน์ Balance เป็นตัวเลข และลบค่าที่แปลงไม่ได้ออก
...
all_results = []
with st.expander("🔍 ตารางวิเคราะห์", expanded=not hunting_symbol):
    with st.spinner("🕵️ Pepper Scanning..."):
        # ปรับวิธีจัดการ data download เพื่อแก้ Warning
    ...
    if all_results:
        ...
        # แก้ไข width='stretch' ตามคำแนะนำใหม่ของ Streamlit
...
if not hunting_symbol and all_results:
    ...
    if best_coin['Score'] >= 80:
        ...
        st.success(f"🛡️ ซื้อแล้ว: {best_coin['Symbol']} ทุน {BASE_INVESTMENT} ฿")
        st.rerun()
